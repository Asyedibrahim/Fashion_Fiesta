...
export default function DashCategory() {
  ...
  const [formData, setFormData] = useState({ cname: "", id:"" });
  ...
  <div className='max-w-4xl w-full p-3'>
    <h1 className='text-3xl mt-5'>Category</h1>
      ...
      <div className='mt-10 flex  gap-3'>
        ...
        <button className='bg-[#ff008a] hover:bg-[#ff319f] text-white px-3 rounded-md text-sm font-semibold' type='submit' onClick={handleSubmit}>{formData.id ? 'Update' : 'Create'}</button>
      </div>
            ...
            {categories.map((category) => (
              <Table.Body className='divide-y' key={category._id}>
                <Table.Row className='bg-white' >
                  ...
                  <Table.Cell className='text-teal-500 font-medium hover:underline cursor-pointer' onClick={() => handleEdit(category._id, category.cname)}>
                    Edit
                  </Table.Cell>
                  <Table.Cell>
